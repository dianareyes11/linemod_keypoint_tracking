...
void PoseDetection::detect(std::vector<cv::Mat>& in_imgs, std::string const& in_className,
                           ...
                           bool in_displayResults)
{
	...
	translateImg(correctedTranslationColor, -camParams.cx + camParams.videoWidth / 2,
	...
	if (!detectedPoses.empty())
	{
		...
		for (auto& detectedPose : detectedPoses)
		{
			...
			{
				uint16_t bestPose = 0;
				icp->prepareDepthForIcp(depthImg, camParams.cameraMatrix,
				                        detectedPose[0].boundingBox);
				icp->registerToScene(detectedPose, numClassIndex);
				bool truePositivMatch = icp->estimateBestMatch(
					correctedTranslationDepth, detectedPose, opengl, numClassIndex, bestPose);
				if (truePositivMatch)
				{
...
void PoseDetection::readLinemodFromFile()
{
	...
	if (ids != modelFiles)
	{
		std::cout << "ERROR::Models in file folder do not match with generated models!" << std::
			endl;
	}
}

void PoseDetection::drawCoordinateSystem(cv::Mat& in_srcDstImage, const cv::Mat& in_camMat,
                                         ...
                                         ObjectPose& in_objPos)
{
	...
	cv::projectPoints(coordinatePoints, rVec, tVec, in_camMat, std::vector<double>(),
	                  projectedPoints);
